...
DEFAULT_SPEED_URL = "https://www.gstatic.com/charts/loader.js?cb={bytes}"
...
def run_speed_tests(v: VlessConfig, ok_results: List[ScanResult], xray: Path, workers: int, timeout: int, speed_bytes: int, speed_duration: int, speed_url_template: str, loglevel: str) -> List[ScanResult]:
    ...
    if RICH:
        ...
        console.print(Panel(title, border_style="yellow", expand=False))
        progress = Progress(SpinnerColumn(), BarColumn(), TextColumn("{task.completed}/{task.total}"), TimeElapsedColumn(), console=console)
        with progress:
            task = progress.add_task("Progress", total=len(targets))
...
def resume_checkpoint_path(output_dir: Path) -> Path:
    return output_dir / "scan_resume.json"


def write_resume_checkpoint(output_dir: Path, targets: List[str], results: List[ScanResult], target_label: str) -> Path:
    """Store unfinished targets so the next run can continue the scan."""
    output_dir.mkdir(parents=True, exist_ok=True)
    done = {r.ip for r in results}
    remaining = [ip for ip in targets if ip not in done]
    payload = {
        "version": APP_VERSION,
        "created_at": datetime.now().isoformat(timespec="seconds"),
        "target_label": target_label,
        "total_targets": len(targets),
        "done_targets": len(done),
...
def load_resume_checkpoint(output_dir: Path) -> Optional[Dict[str, object]]:
    path = resume_checkpoint_path(output_dir)
    if not path.exists():
        return None
    try:
        data = json.loads(path.read_text(encoding="utf-8"))
        remaining = data.get("remaining_targets") or []
...
def clear_resume_checkpoint(output_dir: Path) -> None:
    try:
        resume_checkpoint_path(output_dir).unlink(missing_ok=True)
    except Exception:
        pass


def vless_with_ip(v: VlessConfig, ip: str) -> str:
    """Return a VLESS URI with only the outbound host replaced by the clean IP."""
    parsed = urlparse(v.raw)
    port = f":{parsed.port}" if parsed.port else ""
    netloc = f"{v.uuid}@{ip}{port}"
    return urlunparse((parsed.scheme or "vless", netloc, parsed.path, parsed.params, parsed.query, parsed.fragment))


def final_score(r: ScanResult, max_speed: float, min_latency: float) -> float:
    latency = r.latency_ms if r.latency_ms and r.latency_ms > 0 else None
    speed = r.speed_mbps if r.speed_mbps and r.speed_mbps > 0 else 0.0
    latency_score = (min_latency / latency * 100.0) if latency and min_latency > 0 else 0.0
    stability_score = (r.recheck_passed / r.recheck_total * 100.0) if r.recheck_total else 0.0
    speed_score = (speed / max_speed * 100.0) if max_speed > 0 else 0.0
    score = latency_score * 0.45 + stability_score * 0.25 + speed_score * 0.30
    return max(0.0, min(100.0, score))


def rank_final_results(results: List[ScanResult]) -> List[Tuple[ScanResult, float]]:
    ok = [r for r in results if r.ok]
    latencies = [r.latency_ms for r in ok if r.latency_ms and r.latency_ms > 0]
    speeds = [r.speed_mbps for r in ok if r.speed_mbps and r.speed_mbps > 0]
    min_latency = min(latencies) if latencies else 0.0
    max_speed = max(speeds) if speeds else 0.0
    ranked = [(r, final_score(r, max_speed, min_latency)) for r in ok]
    ranked.sort(key=lambda item: (-item[1], item[0].ip))
    return ranked


def save_final_ranked_configs(v: VlessConfig, results: List[ScanResult], output_dir: Path, prefix: str = "final_ranked_configs") -> Tuple[Path, Path, Path]:
    """Save scored IPs plus ready-to-use VLESS configs."""
    export_dir = output_dir / "exports"
    export_dir.mkdir(parents=True, exist_ok=True)
    ranked = rank_final_results(results)
    txt_path = export_dir / f"{prefix}.txt"
    csv_path = export_dir / f"{prefix}.csv"
    configs_path = export_dir / "best_configs.txt"
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["rank", "ip", "score", "latency_ms", "recheck_passed", "recheck_total", "speed_mbps", "config", "created_at"])
        for idx, (r, score) in enumerate(ranked, 1):
            cfg = vless_with_ip(v, r.ip)
            latency = f"{r.latency_ms:.1f}" if r.latency_ms is not None else ""
            speed = f"{r.speed_mbps:.2f}" if r.speed_mbps is not None else ""
            pass_text = f"{r.recheck_passed}/{r.recheck_total}" if r.recheck_total else "-"
            txt_lines.append(f"#{idx} {r.ip} | score: {score:.1f} | latency: {latency or '-'} ms | pass: {pass_text} | speed: {speed or '-'} Mbps")
            txt_lines.append(cfg)
            txt_lines.append("")
            config_lines.append(cfg)
            w.writerow([idx, r.ip, f"{score:.1f}", latency, r.recheck_passed or "", r.recheck_total or "", speed, cfg, now])
    if not ranked:
        txt_lines.append("No final ranked configs available.")
    txt_path.write_text("\n".join(txt_lines).rstrip() + "\n", encoding="utf-8")
    configs_path.write_text("\n".join(config_lines) + ("\n" if config_lines else ""), encoding="utf-8")
    return txt_path, csv_path, configs_path


def save_ip_export(results: List[ScanResult], output_dir: Path, name: str, filename_prefix: str) -> Path:
    export_dir = output_dir / "exports"
    export_dir.mkdir(parents=True, exist_ok=True)
    rows = sorted([r for r in results if r.ok], key=output_sort_key(filename_prefix))
    path = export_dir / name
    lines = [r.ip for r in rows]
    path.write_text("\n".join(lines) + ("\n" if lines else ""), encoding="utf-8")
    return path


def export_selection_tui(v: VlessConfig, clean_results: List[ScanResult], recheck_results: List[ScanResult], speed_results: List[ScanResult], output_dir: Path) -> None:
    rows = [
        ("Clean IPs only", "Export IP list from the first latency scan"),
        ("Re-checked IPs", "Export IP list from latency re-check"),
        ("Speed-tested IPs", "Export IP list sorted by speed"),
        ("Final ranked configs", "Export scored IPs and ready VLESS configs"),
        ("All", "Export every available final output"),
    ]
    idx = key_select_menu("What do you want to export?", rows, default_index=3, allow_back=False)
    created: List[Path] = []
    if idx in (0, 4):
        created.append(save_ip_export(clean_results, output_dir, "selected_clean_ips.txt", "clean_ips"))
    if idx in (1, 4) and recheck_results:
        created.append(save_ip_export(recheck_results, output_dir, "selected_rechecked_ips.txt", "clean_ips_rechecked"))
    if idx in (2, 4) and speed_results:
        created.append(save_ip_export(speed_results, output_dir, "selected_speed_tested_ips.txt", "clean_ips_speed_tested"))
    if idx in (3, 4):
        base = speed_results or recheck_results or clean_results
        created.extend(save_final_ranked_configs(v, base, output_dir))
    render_stage("Export Complete", "Selected output files were saved.", "green")
    if RICH:
...
def run_latency_recheck(v: VlessConfig, ok_results: List[ScanResult], xray: Path, workers: int, timeout: int, sample_count: int, url: str, loglevel: str) -> List[ScanResult]:
    ...
    if RICH:
        ...
        progress = Progress(SpinnerColumn(), BarColumn(), TextColumn("{task.completed}/{task.total}"), TimeElapsedColumn(), console=console)
        with progress:
            task = progress.add_task("Progress", total=len(ips))
            ex = ThreadPoolExecutor(max_workers=workers)
            futures = {}
            try:
    ...
    while True:
        ...
        while True:
            ...
            while True:
                ...
                resume_data = load_resume_checkpoint(output_dir)
                if resume_data:
                    remaining = [str(x) for x in (resume_data.get("remaining_targets") or [])]
                    done_count = int(resume_data.get("done_targets") or 0)
                    total_count = int(resume_data.get("total_targets") or (done_count + len(remaining)))
                    render_stage("Resume Scan", f"Found interrupted scan: {done_count}/{total_count} done, {len(remaining)} remaining.", "yellow")
                    try:
                        if remaining and prompt_confirm_nav("Resume from the saved checkpoint?", True, allow_back=True):
                            targets = remaining
                            target_label = f"Resume: {resume_data.get('target_label', target_label)}"
                        else:
                ...
                try:
                    ...
                    resume_path = write_resume_checkpoint(output_dir, targets, results, target_label)
                    txt, csvp = save_results(results, output_dir, "clean_ips_interrupted")
                    render_stage("Scan Interrupted", f"Ctrl+C detected. Partial results were saved and resume checkpoint was written: {resume_path}", "yellow")
                    show_final_results(results, txt, csvp)
                    return
                show_final_results(results, txt, csvp)

                clean_stage_results = [r for r in results if r.ok]
                recheck_stage_results: List[ScanResult] = []
                speed_stage_results: List[ScanResult] = []
                final_results = clean_stage_results[:]
                ...
                if do_recheck:
                    ...
                    if sample_count and recheck_workers:
                        ...
                        try:
                            ...
                            recheck_stage_results = final_results[:]
                ...
                if do_speed:
                    ...
                    if do_speed:
                        ...
                        try:
                            ...
                            speed_stage_results = final_results[:]
                            ...
                            return
                        show_final_results(final_results, stxt, scsv, filename_prefix="clean_ips_speed_tested")
                        export_selection_tui(v, clean_stage_results, recheck_stage_results, speed_stage_results, output_dir)
...
def run_scan(v: VlessConfig, targets: List[str], xray: Path, concurrency: int, timeout: int, tries: int, url: str, loglevel: str, keep_configs: bool) -> List[ScanResult]:
    results: List[ScanResult] = []
    if RICH:
        console.print(Panel("Scanning targets", border_style="cyan", expand=False))
        progress = Progress(SpinnerColumn(), BarColumn(), TextColumn("{task.completed}/{task.total}"), TimeElapsedColumn(), console=console)
        with progress:
            task = progress.add_task("Progress", total=len(targets))
