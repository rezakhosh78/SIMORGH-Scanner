...
type StartRequest struct {
	...
	ISPRanges        map[string][]string `json:"isp_ranges"`
...
var clientRequestJobs = map[string]string{}
var backendInstanceID = randHex(6)
var backendStartedAt = time.Now()
...
func main() {
	...
	http.HandleFunc("/api/isp-ranges", handleISPRanges)
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "SIMORGH backend listen error:", err)
		return
	}
	// Recover only after this process owns the local port. This prevents a
	// duplicate backend from resuming the same scan while an older process is
	// still alive but briefly busy.
	recoverInterruptedJobs()
	if err := server.Serve(listener); err != nil && err != http.ErrServerClosed {
		fmt.Fprintln(os.Stderr, "SIMORGH backend server error:", err)
	}
}
...
func handleHealth(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, map[string]any{
		"ok": true, "service": "simorgh-go", "time": time.Now().UnixMilli(),
		"instance_id": backendInstanceID, "started_at": backendStartedAt.UnixMilli(), "active_jobs": active,
	})
}

func jobStatePath(id string) string {
	return filepath.Join(jobsDir, id, "state.json")
}

func writeJobState(j *Job, state string) {
	...
	path := jobStatePath(j.ID)
	tmp := path + ".tmp"
	b, err := json.Marshal(PersistedJobState{State: state, UpdatedAt: time.Now().UnixMilli()})
	if err != nil {
		return
	}
...
func readJobState(id string) PersistedJobState {
	var st PersistedJobState
	b, err := os.ReadFile(jobStatePath(id))
	if err == nil {
...
func appendLogPath(path, line string) {
...
func recoverInterruptedJobs() {
	...
	for _, entry := range entries {
		...
		if st.UpdatedAt > 0 && now.Sub(time.UnixMilli(st.UpdatedAt)) > 6*time.Hour {
			continue
		}
		candidates = append(candidates, candidate{id: id, updated: st.UpdatedAt})
	}
	sort.Slice(candidates, func(i, k int) bool { return candidates[i].updated > candidates[k].updated })
	// Only the most recent active scan is restored. Older interrupted scans stay
	// on disk and can still be inspected from Logs/Files.
	if len(candidates) > 0 {
		if err := restoreInterruptedJob(candidates[0].id); err != nil {
			j := &Job{ID: candidates[0].id}
			writeJobState(j, "failed")
			appendLogPath(filepath.Join(jobsDir, candidates[0].id, "job.log"), "[go] interrupted job recovery failed: "+err.Error())
		}
	}
}

func restoreScanCheckpoint(j *Job) {
	f, err := os.Open(scanCheckpointPath(j))
	if err != nil {
		return
	}
	defer f.Close()
	...
	for scanner.Scan() {
		...
		j.ScanDone[ip] = true
		if strings.EqualFold(parts[2], "OK") {
			lat, _ := strconv.ParseFloat(parts[3], 64)
			if lat > 0 {
				j.Rows[ip] = &Row{IP: ip, LatencyMS: lat, Phase: "SCAN"}
			}
		}
	}
}

func restoreLegacyScanLog(j *Job, path string) {
	f, err := os.Open(path)
	if err != nil {
		return
	}
	defer f.Close()
	okRE := regexp.MustCompile(`^OK\s+(\S+)\s+([0-9.]+)\s+ms$`)
	...
	for scanner.Scan() {
		...
		if m := okRE.FindStringSubmatch(line); len(m) == 3 {
			lat, _ := strconv.ParseFloat(m[2], 64)
			j.ScanDone[m[1]] = true
			if lat > 0 {
				j.Rows[m[1]] = &Row{IP: m[1], LatencyMS: lat, Phase: "SCAN"}
			}
			continue
		}
...
func restoreInterruptedJob(id string) error {
	payloadPath := filepath.Join(jobsDir, id, "payload.json")
	...
	normalize(&req)
	targets := readLines(filepath.Join(targetsDir, "targets_"+id+".txt"))
	if len(targets) == 0 {
		return fmt.Errorf("targets are missing")
	}

	logPath := filepath.Join(jobsDir, id, "job.log")
	logText := readTail(logPath, 1200000)
	if strings.Contains(logText, "[go] job finished") {
		writeJobState(&Job{ID: id}, "finished")
		return nil
	}
	if strings.Contains(logText, "job stopped by user") {
		writeJobState(&Job{ID: id}, "stopped")
		return nil
	}
	if strings.Contains(logText, "job cancelled by user") {
		writeJobState(&Job{ID: id}, "cancelled")
		return nil
	}
	events := parseEvents(logText, filepath.Join(resultsDir, id))
	ctx, cancel := context.WithCancel(context.Background())
	j := &Job{
		ID: id, Req: req, Dir: filepath.Join(jobsDir, id), ResultDir: filepath.Join(resultsDir, id),
		LogPath: logPath, Ctx: ctx, Cancel: cancel, Running: true, Started: time.Now(),
		Rows: map[string]*Row{}, Targets: append([]string(nil), targets...), ScanDone: map[string]bool{},
		RecheckDone: map[string]bool{}, SpeedDone: map[string]bool{},
	}
	jobInitProgress(j, len(targets))
	restoreScanCheckpoint(j)
	if len(j.ScanDone) == 0 {
		restoreLegacyScanLog(j, logPath)
	}
	for _, ev := range events {
		if strings.TrimSpace(ev.IP) == "" || ev.LatencyValue <= 0 {
			continue
		}
		row := &Row{IP: ev.IP, LatencyMS: ev.LatencyValue, AvgLatencyMS: ev.AvgLatencyValue, RecheckPassed: ev.PassValue, RecheckTotal: ev.PassTotal, SpeedMbps: ev.SpeedValue, Phase: ev.Phase}
		j.Rows[ev.IP] = row
		j.ScanDone[ev.IP] = true
		if ev.PassTotal > 0 {
			j.RecheckDone[ev.IP] = true
		}
		if ev.SpeedValue > 0 {
			j.SpeedDone[ev.IP] = true
		}
	}
	j.Progress.Scan = phaseProgress("scan", len(j.ScanDone), len(targets))
	...
	j.Progress.CleanCount = len(j.Rows)
	j.Progress.FailCount = max(0, len(j.ScanDone)-len(j.Rows))
	...
	if req.ClientRequestID != "" {
		clientRequestJobs[req.ClientRequestID] = id
	}
	jobsMu.Unlock()
	appendLogPath(logPath, "[go] backend recovered interrupted job; resuming unfinished targets")
	writeJobState(j, "running")
	go runJob(j)
	return nil
}
...
func handleStart(w http.ResponseWriter, r *http.Request) {
	...
	jobsMu.Lock()
	if req.ClientRequestID != "" {
		if existingID := clientRequestJobs[req.ClientRequestID]; existingID != "" {
			if existing := jobs[existingID]; existing != nil {
				jobsMu.Unlock()
				cancel()
				writeJSON(w, map[string]any{"ok": true, "job_id": existingID, "reused": true})
				return
			}
		}
		clientRequestJobs[req.ClientRequestID] = id
...
func runJob(j *Job) {
	...
	defer func() {
		...
		finalRC := rc
		if j.ReturnCode != nil {
			finalRC = *j.ReturnCode
		}
		j.mu.Unlock()
		if stage == "finished" && finalRC == 0 {
			writeJobState(j, "finished")
		} else if finalRC != 130 {
			writeJobState(j, "failed")
		}
	}()
...
func targetTokens(data []byte) []string {
...
func collectTargets(req StartRequest) ([]string, error) {
	...
	if strings.ToLower(req.Mode) == "isp" {
		...
		rangeAllow := map[string][]string{}
		for id, ranges := range req.ISPRanges {
		...
		if !req.ISPAll && len(fullAllow) == 0 && len(rangeAllow) == 0 {
			return nil, fmt.Errorf("no ISP selected; select an ISP or one of its IP ranges")
		}
		...
		for _, entry := range manifest.Files {
			...
			selectedWhole := req.ISPAll || fullAllow[entry.ID]
			selectedRanges := rangeAllow[entry.ID]
			if !selectedWhole && len(selectedRanges) == 0 {
				continue
			}
			if limit > 0 && len(out) >= limit {
				break
			...
			if !selectedWhole {
				...
				for _, tok := range targetTokens(plain) {
					available[tok] = true
				}
				filtered := make([]string, 0, len(selectedRanges))
				for _, tok := range selectedRanges {
					if available[tok] {
				...
				targetData = []byte(strings.Join(filtered, "\n") + "\n")
			}

			matchedFiles++
			ips, err := parseTargetData(targetData, remaining())
			if err != nil {
		...
		if matchedFiles == 0 {
			return nil, fmt.Errorf("selected ISP or IP ranges were not found in category %s", cat)
		}
	}
	...
	if len(out) == 0 {
		return nil, fmt.Errorf("no valid targets loaded")
	}
	return out, nil
}
...
func scanCheckpointPath(j *Job) string {
	return filepath.Join(j.Dir, "scan.checkpoint")
}

func appendScanCheckpoint(f *os.File, outcome scanOutcome) {
	if f == nil || strings.TrimSpace(outcome.IP) == "" {
		return
	}
	if outcome.Err == nil && outcome.Latency > 0 {
		_, _ = fmt.Fprintf(f, "v1\t%s\tOK\t%.6f\n", outcome.IP, outcome.Latency)
	} else {
		_, _ = fmt.Fprintf(f, "v1\t%s\tFAIL\t0\n", outcome.IP)
	}
}

func scanProgressTotals(j *Job) (int, int) {
	j.mu.Lock()
	defer j.mu.Unlock()
	return j.Progress.Scan.Done, j.Progress.Scan.Total
}

func scanTargets(j *Job, logf *os.File, targets []string) []Row {
	workers := clampInt(j.Req.Concurrency.Int(), 1, maxScanWorkers)
	logLine(logf, fmt.Sprintf("[go] scan workers=%d timeout=%ds target_cap=%d engine=shared-xray-batch", workers, j.Req.Timeout.Int(), j.Req.MaxHosts.Int()))
	if len(targets) == 0 {
		return nil
	}

	checkpoint := openAppend(scanCheckpointPath(j))
	if checkpoint != nil {
		defer checkpoint.Close()
	}

	rows := make([]Row, 0)
	timeout := time.Duration(j.Req.Timeout.Int()) * time.Second
	for offset := 0; offset < len(targets); offset += workers {
		if j.Ctx.Err() != nil {
			break
		}
		end := min(len(targets), offset+workers)
		batchIPs := append([]string(nil), targets[offset:end]...)
		cmd, batchTargets, batchLogPath, err := startXrayBatch(j.Ctx, j, batchIPs, timeout)
		if err != nil {
			logLine(logf, fmt.Sprintf("[go] scan batch startup failed size=%d error=%v", len(batchIPs), err))
			for _, ip := range batchIPs {
				outcome := scanOutcome{IP: ip, Err: err}
				appendScanCheckpoint(checkpoint, outcome)
				logLine(logf, "FAIL "+ip)
				done := jobScanResult(j, ip, nil, false)
				_, total := scanProgressTotals(j)
				if done%250 == 0 || done == total {
					logLine(logf, fmt.Sprintf("SCAN progress: %d/%d", done, total))
				}
			}
			if checkpoint != nil {
				_ = checkpoint.Sync()
		...
		outcomes := make(chan scanOutcome, len(batchTargets))
		...
		for _, target := range batchTargets {
			...
			go func() {
				...
				defer func() {
					if v := recover(); v != nil {
						outcomes <- scanOutcome{IP: target.IP, Err: fmt.Errorf("scan panic: %v", v)}
					}
				}()
				lat, status, probeErr := probeThroughProxy(j.Ctx, j, target.Port, timeout, batchLogPath)
				outcomes <- scanOutcome{IP: target.IP, Latency: lat, Status: status, Err: probeErr}
			}()
		}
		go func() {
			wg.Wait()
		...
		for outcome := range outcomes {
			appendScanCheckpoint(checkpoint, outcome)
			if outcome.Err == nil {
				row := Row{IP: outcome.IP, LatencyMS: outcome.Latency, Phase: "SCAN"}
				jobScanResult(j, outcome.IP, &row, true)
				rows = append(rows, row)
				logLine(logf, fmt.Sprintf("OK %s %.1f ms", outcome.IP, outcome.Latency))
			} else {
				logLine(logf, "FAIL "+outcome.IP)
				jobScanResult(j, outcome.IP, nil, false)
			}
			done, total := scanProgressTotals(j)
			if done%250 == 0 || done == total {
				logLine(logf, fmt.Sprintf("SCAN progress: %d/%d", done, total))
			}
		}
	...
	sort.Slice(rows, func(i, k int) bool { return rows[i].LatencyMS < rows[k].LatencyMS })
	return rows
}
...
type xrayBatchTarget struct {
	IP   string
	Port int
}

func buildVLESSOutbound(vc *VLESSConfig, ip, tag string) map[string]any {
	user := map[string]any{
		"id":         vc.ID,
		"encryption": "none",
	}
...
func buildXrayBatchConfig(req StartRequest, targets []xrayBatchTarget) ([]byte, error) {
	...
	if len(targets) == 0 {
		return nil, fmt.Errorf("empty Xray batch")
	}
	...
	for i, target := range targets {
		inTag := fmt.Sprintf("scan-in-%d", i)
		outTag := fmt.Sprintf("scan-out-%d", i)
		...
		outbounds = append(outbounds, buildVLESSOutbound(vc, target.IP, outTag))
...
func buildXrayConfig(req StartRequest, ip string, localPort int) ([]byte, error) {
	return buildXrayBatchConfig(req, []xrayBatchTarget{{IP: ip, Port: localPort}})
}
...
func allocatePorts(count int) ([]int, error) {
	...
	for len(ports) < count {
		...
		if attempts > count*20+100 {
			return nil, fmt.Errorf("could not allocate %d unique local ports", count)
		}
		p, err := freePort()
		if err != nil {
			return nil, err
...
func waitForPorts(ctx context.Context, ports []int, timeout time.Duration) error {
	if len(ports) == 0 {
		return fmt.Errorf("no local proxy ports")
	}
	...
	for len(pending) > 0 && time.Now().Before(deadline) {
		...
		for p := range pending {
			...
			c, err := net.DialTimeout("tcp", addr, 45*time.Millisecond)
			if err == nil {
				_ = c.Close()
				delete(pending, p)
			}
	...
	return fmt.Errorf("xray batch proxy did not start on %d/%d ports", len(pending), len(ports))
}
...
func startXrayBatch(ctx context.Context, j *Job, ips []string, timeout time.Duration) (*exec.Cmd, []xrayBatchTarget, string, error) {
	ports, err := allocatePorts(len(ips))
	if err != nil {
		return nil, nil, "", err
	}
	targets := make([]xrayBatchTarget, 0, len(ips))
	for i, ip := range ips {
		targets = append(targets, xrayBatchTarget{IP: ip, Port: ports[i]})
	}

	cfg, err := buildXrayBatchConfig(j.Req, targets)
	if err != nil {
		return nil, nil, "", err
	}

	stamp := strconv.FormatInt(time.Now().UnixNano(), 36)
	cfgPath := filepath.Join(j.Dir, "xray_scan_batch_"+stamp+".json")
	if err := os.WriteFile(cfgPath, cfg, 0600); err != nil {
		return nil, nil, "", err
	}

	cmd := exec.CommandContext(ctx, xrayPath, "run", "-config", cfgPath)
	cmd.Dir = baseDir
	logPath := "discarded"
	var logFile *os.File
	level := safeLogLevel(j.Req.LogLevel)
	if j.Req.KeepConfigs || level == "debug" || level == "info" {
		logPath = filepath.Join(j.Dir, "xray_scan_batch_"+stamp+".log")
	...
	startupTimeout := clampDuration(timeout/2, 2500*time.Millisecond, 8*time.Second)
	if err := waitForPorts(ctx, ports, startupTimeout); err != nil {
		_ = cmd.Process.Kill()
	...
	return cmd, targets, logPath, nil
}
...
func probeThroughProxy(ctx context.Context, j *Job, port int, timeout time.Duration, logPath string) (float64, int, error) {
	if timeout <= 0 {
	...
	for _, target := range latencyProbeURLs(j.Req.URL) {
		...
		lat := float64(time.Since(started).Microseconds()) / 1000.0
...
func probeXray(ctx context.Context, j *Job, ip string, timeout time.Duration) (float64, int, error) {
	...
	defer stopXray(cmd)
	return probeThroughProxy(probeCtx, j, port, timeout, logPath)
}
...
func speedTestXray(ctx context.Context, j *Job, ip string) (float64, string, error) {
	...
	for totalBytes < targetBytes && time.Now().Before(hardDeadline) {
		...
		req.Header.Set("User-Agent", "SIMORGH-Scanner/0.3.1")
...
func handleStatus(w http.ResponseWriter, r *http.Request) {
	...
	if j != nil {
		...
		state := strings.ToLower(strings.TrimSpace(progress.Stage))
		if running {
			state = "running"
		}
		definitive := !running && (state == "finished" || state == "stopped" || state == "cancelled" || rc != nil)
		writeJSON(w, map[string]any{
			"ok": true, "found": true, "definitive": definitive, "state": state,
			"job_id": id, "running": running, "returncode": rc, "events": events,
			"progress": progress, "files": resultFiles(id), "backend_instance": backendInstanceID,
		})
		return
	}
	...
	if id == "" {
		writeJSON(w, map[string]any{
			"ok": true, "found": false, "definitive": false, "state": "no_job",
			"job_id": "", "running": false, "returncode": nil, "events": []Event{},
			"progress": Progress{}, "files": []FileInfo{}, "backend_instance": backendInstanceID,
		})
		return
	}
	...
	persisted := readJobState(id)
	state := strings.ToLower(strings.TrimSpace(persisted.State))
	definitive := false
	var rc any = nil
	switch state {
	case "finished":
	...
	default:
		// A job missing from memory without an explicit terminal state usually
		// means the native backend was restarted. Never report this as Completed.
		state = "recovering"
	}
	writeJSON(w, map[string]any{
		"ok": true, "found": false, "definitive": definitive, "state": state,
		"job_id": id, "running": false, "returncode": rc, "events": events,
		"progress": progress, "files": resultFiles(id), "backend_instance": backendInstanceID,
	})
}
...
func handleStop(w http.ResponseWriter, r *http.Request) {
	...
	if j != nil {
		...
		logPath := j.LogPath
		j.Cancel()
		writeJobState(j, "stopped")
		appendLogPath(logPath, "[go] job stopped by user")
	}
...
func handleCancel(w http.ResponseWriter, r *http.Request) {
	...
	if j != nil {
		...
		logPath := j.LogPath
		j.Cancel()
		writeJobState(j, "cancelled")
		appendLogPath(logPath, "[go] job cancelled by user")
	}
	delete(jobs, id)
	jobsMu.Unlock()
...
func handleISPRanges(w http.ResponseWriter, r *http.Request) {
	...
	if id == "" {
		writeJSON(w, map[string]any{"ok": false, "ranges": []string{}, "error": "ISP id is required"})
		return
	}

	manifest, err := loadISPManifest()
	if err != nil {
		writeJSON(w, map[string]any{"ok": false, "ranges": []string{}, "error": err.Error()})
		return
	}

	for _, entry := range manifest.Files {
		if entry.Category != cat || entry.ID != id {
			continue
		}
		plain, err := loadISPEntry(entry)
		if err != nil {
	...
	writeJSON(w, map[string]any{"ok": false, "ranges": []string{}, "error": "ISP was not found"})
}
...
func handleLogs(w http.ResponseWriter, r *http.Request) {
	...
	logText := readTail(filepath.Join(jobsDir, id, "job.log"), 240000)
